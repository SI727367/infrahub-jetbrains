...
`YamlTreePanel` parses `.infrahub.yml` or `.infrahub.yaml`, renders sections like queries, transforms, artifact definitions, generators, and checks, and resolves linked file paths where present. Its popup menu can execute GraphQL queries and run infrahubctl transforms for selected transform items or artifact-linked transformations. It also subscribes to VFS change events so changes to either supported Infrahub project file name trigger an automatic refresh.
